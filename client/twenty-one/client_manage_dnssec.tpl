	...
	{if isset($dnssec.keys)}
		{foreach from=$dnssec.keys item=key}
			<tr>
				...
				<td align="center"> {$key.flag} </td>
				<td align="center"> {if $key.active eq '1'} <span class="label status status-active">active</span> {else} <span class="label status">inactive</span> {/if} </td>
